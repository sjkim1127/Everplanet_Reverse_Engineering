...
function patchLoginAuthFunctions() {
  try {
    ...
    Interceptor.attach(loginWrapperAddr, {
      ...
      onLeave(retval) {
        log("[LoginWrapper FUN_00755280] Original return: " + retval);
    ...
    Interceptor.attach(loginUIAddr, {
      ...
      onLeave(retval) {
        log("[LoginUI FUN_007554e0] Original return: " + retval);
...
function applyLobbyPatches() {
  try {
    ...
    try {
      ...
      // [TEST COMMENT OUT] addr3.writeByteArray([0xB8, 0x01, 0x00, 0x00, 0x00, 0xC3]);  // mov eax,1; ret
      log("[+] 0x007551F0 memory patch bypassed. Attaching Interceptor instead.");
      Interceptor.attach(addr3, {
        onEnter(args) {
          log("[0x007551F0] Called");
        },
        onLeave(retval) {
          log("[0x007551F0] Original return: " + retval);
          log("[0x007551F0] Forcing return 1");
          retval.replace(ptr(1));
        }
      });
    } catch(e) { log("[!] Interceptor attach 0x007551F0 failed: " + e); }
    ...
    try {
      ...
      // [TEST COMMENT OUT] addr4.writeByteArray([0xB8, 0x01, 0x00, 0x00, 0x00, 0xC2, 0x04, 0x00]);  // mov eax,1; ret 4
      log("[+] 0x00755280: skipped memory patch");
